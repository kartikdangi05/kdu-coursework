...
function addPost(postContent){
    ...
    postContentName.innerHTML = 'Nitesh Gupta <span style="font-weight: 400; color: rgb(113, 118, 123);">@nit_hck · 34s</span> <span style="font-weight: 700; position: absolute; right: 20px; color: rgb(113, 118, 123);"> ··· </span>';
...
postBtn.addEventListener('click', () => {
    ...
    postContent = postContent.replace(hashtagRegex, '<span style="color: rgb(29, 155, 240);">$1</span>');
    addPost(postContent)

});

postBtnDesk.addEventListener('click', () => {
...
const postContent = document.querySelector('.actual-post-content');
let postText = postContent.textContent;
const hashtagRegex = /#(\w+)/g;
postText = postText.replace(hashtagRegex, '<span class="hashtag">$&</span>');
